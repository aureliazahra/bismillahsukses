...
# Backend: InsightFace
# Perbaikan:
# 1) Normalisasi OFF aliases (off/none/disable/disabled/mati/mati_semua/false/no/0) -> benar2 mematikan liveness & filter berbasis anti-spoof.
# 2) Semua filter layar/HP & suppress wajah kecil hanya aktif bila anti-spoof aktif (bukan off).
# 3) Tambah dukungan heuristic_low dan heuristic_high agar konsisten dengan opsi GUI.
# 4) GPU-aware: pilih ORT providers dinamis (TensorRT -> CUDA -> DML -> CPU) dan set ctx_id otomatis.
# 5) Anti-spoof ONNX ikut providers yang sama; fallback aman bila GPU tidak tersedia.
# 6) Perbaikan bug: _file_ -> __file__.
...
_antispoof_mode = "high"  # default lebih ketat
_antispoof_preprocess = "rgb01"
_antispoof_smooth_k = 3

# OFF aliases
_OFF_ALIASES = {
    "off", "none", "disable", "disabled", "mati", "mati_semua", "false", "no", "0"
}
...
def _pick_providers(use_gpu: bool = True):
    if not use_gpu or ort is None:
...
def _gpu_like_selected(providers):
    return any(p in providers for p in ('TensorrtExecutionProvider', 'CUDAExecutionProvider'))
...
def set_antispoof_params(live_index=None, thresholds=None, mode=None, preprocess=None, smooth_k=None):
    ...
    if live_index is not None:
        try:
            _antispoof_live_index = int(live_index)
        except Exception:
            pass
    if isinstance(thresholds, dict):
        for k, v in thresholds.items():
            try:
                _antispoof_thresholds[k] = float(v)
            except Exception:
                pass
    ...
    if smooth_k is not None:
        try:
            _antispoof_smooth_k = max(1, int(smooth_k))
        except Exception:
            pass
...
def initialize(use_gpu: bool = False, model_name: str = "buffalo_l", detector_backend: str = "insightface"):
    ...
    if INSIGHTFACE_AVAILABLE:
        try:
            providers = _pick_providers(use_gpu=_use_gpu)
            ctx_id = 0 if _gpu_like_selected(providers) else -1

            # Logging bantuan
            try:
                logging.info("ORT available providers: %s", _available_ort_providers())
            except Exception:
                pass
            logging.info("ORT selected providers: %s | ctx_id=%s", providers, ctx_id)

            _app = FaceAnalysis(name=model_name, providers=providers)
            _app.prepare(ctx_id=ctx_id, det_size=(640, 640))

            # Longgarkan ambang krn ingin wajah jauh
            try:
...
def _screen_like_heuristic(bgr_full, bbox, face_crop=None):
    ...
    Aktif hanya ketika anti-spoof aktif.
    """
    try:
...
def process_frame_for_faces(
    ...
    Catatan penting:
      - Jika anti-spoof OFF (alias apapun), liveness & filter berbasis anti-spoof TIDAK dieksekusi.
    """
    ...
    for d in dets:
        ...
        if backend in _OFF_ALIASES:
            backend = "off"
        antispoof_enabled = (backend != "off")
        ...
        # ---- Liveness (hanya bila anti-spoof aktif) ----
        antispoof_checked = False
        antispoof_live = True
        if antispoof_enabled and (bgr_face is not None and bgr_face.size > 0):
            if backend in ("heuristic", "heuristik", "heuristic_low", "heuristik_low", "heuristic-low"):
                antispoof_checked = True
                gray = cv2.cvtColor(bgr_face, cv2.COLOR_BGR2GRAY)
                # low: sangat longgar
                antispoof_live = (gray.mean() >= 15.0)
            elif backend in ("heuristic_medium", "heuristik_medium", "heuristic-medium"):
                antispoof_checked = True
                gray = cv2.cvtColor(bgr_face, cv2.COLOR_BGR2GRAY)
                lap_var = float(cv2.Laplacian(gray, cv2.CV_64F).var())
                antispoof_live = (gray.mean() >= 25.0) and (lap_var >= 55.0)
            elif backend in ("heuristic_high", "heuristik_high", "heuristic-high"):
                antispoof_checked = True
                gray = cv2.cvtColor(bgr_face, cv2.COLOR_BGR2GRAY)
                lap_var = float(cv2.Laplacian(gray, cv2.CV_64F).var())
                # high: paling ketat
                antispoof_live = (gray.mean() >= 28.0) and (lap_var >= 85.0)
        ...
        # ------ FILTER FOTO/GAMBAR (aktif hanya jika anti-spoof aktif) ------
        if antispoof_enabled and bgr_face is not None and bgr_face.size > 0:
...
def load_antispoof_model(path: str = "", use_gpu: bool = False):
    try:
        ...
        providers = _pick_providers(use_gpu=use_gpu)
        # NOTE: beberapa model tidak kompatibel TensorRT; jika gagal, ORT akan fallback ke CUDA/CPU.
